...
class LongformerEmbeddingsTestSpec extends AnyFlatSpec {
  ...
  "LongformerEmbeddings" should "download, save, and load a model" taggedAs SlowTest in {
    ...
    Benchmark.time("Time to save LongformerEmbeddings pipeline model") {
      pipelineModel.write.overwrite().save("./tmp_longformer_pipeline")
    }
    ...
    val loadedDistilBertModel = LongformerEmbeddings.load("./tmp_longformer_model")
