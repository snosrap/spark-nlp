...
class XlmRoBertaEmbeddings(override val uid: String)
  ...
  /** @group setParam */
  def setModelIfNotSet(spark: SparkSession, tensorflowWrapper: TFWrapper[_], spp: SentencePieceWrapper): XlmRoBertaEmbeddings = {
    if (_model.isEmpty) {
      _model = Some(
  ...
  override def onWrite(path: String, spark: SparkSession): Unit = {
    ...
    val tfWrapper = getModelIfNotSet.tensorflowWrapper

    val _tfWrapper =
      if(tfWrapper.useTFIO)
        tfWrapper.asInstanceOf[TensorflowWrapperWithTfIo]
      else
        tfWrapper.asInstanceOf[TensorflowWrapper]

    writeTensorflowModelV2(path, spark, _tfWrapper, "_xlmroberta", XlmRoBertaEmbeddings.tfFile, configProtoBytes = getConfigProtoBytes)
