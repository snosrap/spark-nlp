...
class RoBertaEmbeddings(override val uid: String)
  ...
  /** @group setParam */
  def setModelIfNotSet(spark: SparkSession, tensorflowWrapper: TFWrapper[_]): RoBertaEmbeddings = {
    if (_model.isEmpty) {
      _model = Some(
...
trait ReadRobertaTensorflowModel extends ReadTensorflowModel {
  ...
  def loadSavedModel(tfModelPath: String, spark: SparkSession, useTfIo: String = "false"): RoBertaEmbeddings = {
